...
class TransposerBlackBox[T <: Data](val dim: Int, val dataType: T) extends BlackBox with HasBlackBoxResource {
  require(isPow2(dim))
  ...
  val io = IO(new Bundle {
    ...
    val io_inRow = Flipped(Decoupled(Vec(dim, dataType)))
    val io_outCol = Decoupled(Vec(dim, dataType))
  })

  addResource("/vsrc/TransposerBlackBox.v")
}

class AlwaysOutTransposer[T <: Data](val dim: Int, val dataType: T) extends Transposer[T] {
  require(isPow2(dim))

  val custom_transposer = Module(new TransposerBlackBox[T](dim, dataType))

  custom_transposer.io.clock := clock
  custom_transposer.io.reset := reset

  custom_transposer.io.io_inRow <> io.inRow
  custom_transposer.io.io_outCol <> io.outCol
